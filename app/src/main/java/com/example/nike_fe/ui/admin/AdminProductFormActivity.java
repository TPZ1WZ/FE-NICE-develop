...
public class AdminProductFormActivity extends AppCompatActivity {
    ...
    private static final String IMAGE_PLACEHOLDER_TEXT = "[Hình ảnh quá lớn để hiển thị dạng text - Đã được lưu]";
    ...
    private String originalBase64Image; // Store the original base64 to avoid lag
        ...
        if (requestCode == PICK_IMAGE_REQUEST && resultCode == RESULT_OK && data != null && data.getData() != null) {
            ...
            try {
                ...
                if (uploadedImageUrl != null) {
                    // Don't put large base64 string into EditText
                    etImageUrl.setText(IMAGE_PLACEHOLDER_TEXT);
    ...
    private void loadProductData() {
        ...
        productApi.getProductById(token, productId).enqueue(new Callback<AdminProduct>() {
            @Override
    ...
    private void populateForm(AdminProduct product) {
        ...
        if (product.getSizes() != null) {
            for (int i = 0; i < chipGroupSizes.getChildCount(); i++) {
                com.google.android.material.chip.Chip chip = (com.google.android.material.chip.Chip) chipGroupSizes
                        .getChildAt(i);
        ...
        if (imageUrl != null && imageUrl.length() > 200 && imageUrl.startsWith("data:image")) {
            // It's likely a base64 string
            originalBase64Image = imageUrl;
            etImageUrl.setText(IMAGE_PLACEHOLDER_TEXT);
        } else {
    ...
    private AdminProduct buildProductFromForm() {
        ...
        } else if (imageUrlInput.equals(IMAGE_PLACEHOLDER_TEXT)) {
            // Placeholder unchanged, use original
            finalImageUrl = originalBase64Image != null ? originalBase64Image : "";
        } else if (imageUrlInput.isEmpty()) {
            // Empty
            finalImageUrl = "https://via.placeholder.com/300x300?text=No+Image";
        } else {
            // User typed a new URL
            finalImageUrl = imageUrlInput;
        }

        // Tạo images list cho backend
        java.util.List<String> imagesList = new java.util.ArrayList<>();
        imagesList.add(finalImageUrl);
        product.setImages(imagesList);
        product.setImage(finalImageUrl); // Giữ để backward compatible
            ...
            public void onResponse(Call<AdminProduct> call, Response<AdminProduct> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        Toast.makeText(AdminProductFormActivity.this,
                                "Tạo thất bại: " + response.code(), Toast.LENGTH_LONG).show();
                    } catch (Exception e) {
                        Toast.makeText(AdminProductFormActivity.this,
    ...
    private void updateProduct(AdminProduct product) {
        ...
        productApi.updateProduct(token, productId, product).enqueue(new Callback<AdminProduct>() {
            @Override
            public void onResponse(Call<AdminProduct> call, Response<AdminProduct> response) {
                layoutLoading.setVisibility(View.GONE);
                layoutContent.setVisibility(View.VISIBLE);
